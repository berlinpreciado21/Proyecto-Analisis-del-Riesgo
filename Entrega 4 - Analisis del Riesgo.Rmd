---
title: "Entrega 4 - Análisis del Riesgo de BOLSAA.MX"
author:
...
```{r Obtener datos, include = FALSE}
# clave <- "BOLSAA.MX"
# getSymbols(clave, from = "2016-06-18",  to = today(), warnings = "FALSE")
# lista <- lapply(clave, function(x) Cl(get(x)))
# precio <- (do.call(merge, lista))
# names(precio) <- clave
# rendimiento <- na.omit(dailyReturn(precio))
```

```{r Correlación con el mercado, warning = FALSE, results = FALSE, echo = FALSE}
# getSymbols("^MXX", from = "2016-06-18", to = today(), warnings = "FALSE")
# rend_mercado <- na.omit(dailyReturn(na.omit(MXX$MXX.Adjusted)))
# datos_corr_merc <- merge.xts(rend_mercado, rendimiento)
# names(datos_corr_merc) <- c("IPC", clave)
# chart.Correlation(na.omit(datos_corr_merc))
# #cor(na.omit(datos_corr_merc))
```
```{r}
datos <- new.env()
clave = "BOLSAA.MX"
